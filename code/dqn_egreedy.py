...
class EpsilonGreedyDQN(DQN):
    ...
    # ==================================================================================================================
    # obs, action, reward, obs_next, done, source
    ...
    def get_q_values(self, obs_in):
        obs = (np.moveaxis(np.asarray(obs_in, dtype=np.float32) / 255.0, 0, -1)).astype(dtype=np.float32)
        feed_dict = {self.tf_vars['obs']: [obs]}
        return self.session.run(self.tf_vars['q_values'], feed_dict=feed_dict)

    # ==================================================================================================================

    def get_latent_features(self, obs_in):
        obs = (np.moveaxis(np.asarray(obs_in, dtype=np.float32) / 255.0, 0, -1)).astype(dtype=np.float32)
        feed_dict = {self.tf_vars['obs']: [obs]}
